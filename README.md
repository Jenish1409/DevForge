...
┌─────────────────────────────────────────────────────────────┐
│                        BACKEND                              │
│               Spring Boot 4 (Docker on Render)              │
│                                                             │
│   ┌─────────────────────────────────────────────────────┐   │
│   │              Security Filter Chain                  │   │
│   │     JWT Filter → Rate Limiter → CORS Config         │   │
│   └───────────────────────┬─────────────────────────────┘   │
│                           │                                 │
│   ┌───────────┐  ┌───────┴───────┐  ┌───────────────────┐  │
│   │   Auth    │  │   Project &   │  │  Dynamic Mock     │  │
│   │ Controller│  │   Endpoint    │  │  Controller        │  │
│   │  /api/v1/ │  │  Management   │  │  /mock/{id}/**    │  │
│   └─────┬─────┘  └───────┬───────┘  └────────┬──────────┘  │
│         │                │                    │             │
│   ┌─────┴────────────────┴────────────────────┴─────────┐   │
│   │              Service Layer                          │   │
│   │  AuthService · ProjectService · MockEndpointService │   │
│   │  OtpService · EmailService · RateLimitingService    │   │
...
```
Client Request: GET /mock/abc-123/users/42
                      ▼
               ┌──────────────┐
               ...
               └──────┬───────┘
                      ▼
               ┌──────────────┐
               │ Redis Cache  │ ← Cache key: "{keyHash}:GET:/users/42"
               │   Lookup     │
               └──┬───────┬───┘
            HIT   │       │ MISS
                  ▼       ▼
              Return   ┌──────────────┐
                       │  DB Lookup   │
                       │ Exact Match  │ ← findByProjectIdAndMethodAndPath
                       │ → AntPath    │ ← pattern: /users/{id}
                       └──────┬───────┘
                              ▼
                       ┌──────────────┐
                       ...
                       └──────┬───────┘
                              ▼
                       ┌──────────────┐
                       │ Apply Delay  │ ← Thread.sleep(delayMs)
                       └──────┬───────┘
                              ▼
                        HTTP Response
                     (status + headers + body)
...
│   │   ├── config/                   # WebConfig, CacheConfig, AsyncConfig
